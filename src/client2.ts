...
import { SandboxServiceClient, SandboxServiceDefinition } from './compiled_contracts/sandbox';
import {
  StopOrdersServiceClient,
  StopOrdersServiceDefinition,
} from './compiled_contracts/stoporders';
import { UsersServiceClient, UsersServiceDefinition } from './compiled_contracts/users';
...
class TTechApiClient {
  ...
  constructor(config: TTechAPIClientOptions) {
    ...
    var clientFactory = createClientFactory().use((call, options) =>
    ...
    this.sandbox = clientFactory.create(SandboxServiceDefinition, channel);
  }
}
