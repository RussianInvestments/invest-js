...
import {
  ...
  OrderBookInstrument,
  TradeInstrument,
} from '@tinkoff/invest-js/build/compiled_contracts/marketdata.js';
...
async function loadShares(): Promise<Share[]> {
  try {
    var sharesResponse = await clientV2.instruments.shares({
  ...
  async function* createRequests(): AsyncIterable<DeepPartial<MarketDataRequest>> {
    ...
    yield MarketDataRequest.create({
      subscribeOrderBookRequest: SubscribeOrderBookRequest.create({
        subscriptionAction: SubscriptionAction.SUBSCRIPTION_ACTION_SUBSCRIBE,
        instruments: shares.map((share) =>
          OrderBookInstrument.create({ instrumentId: share.uid, depth: 10 })
        ),
      }),
    });
    yield MarketDataRequest.create({
...
loadShares().then(connectMdStreams).catch(console.error);
