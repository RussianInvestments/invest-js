...
const columns = [
  ...
  { name: 'Qty', label: 'Quantity', field: (row: any) => row.position.quantity.units },
  {
    name: 'avgPrice',
    label: 'AvgPrice',
    field: (row: any) => row.position.averagePositionPrice.units,
  },
];
...
const getSelectedString = () => {
  return selected.value.length === 0
    ? ''
    : `${selected.value.length} record${selected.value.length > 1 ? 's' : ''} selected of ${positions.value.length}`;
};
...
<template>
  <q-page>
    ...
    <div class="q-pa-md">
      ...
      <div class="q-mt-md">
        Selected: {{ JSON.stringify(selected) }} DATA: {{ JSON.stringify(store.positions) }}
      </div>
    </div>
  </q-page>
</template>
