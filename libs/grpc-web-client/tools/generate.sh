...
output_path=libs/grpc-web-client/compiled

npx grpc_tools_node_protoc \
  --plugin=protoc-gen-ts_proto=node_modules/.bin/protoc-gen-ts_proto \
  --ts_proto_out=$output_path \
  --ts_proto_opt=env=browser,outputServices=nice-grpc,outputServices=generic-definitions,useExactTypes=false,outputJsonMethods=false,exportCommonSymbols=false,outputIndex=false,esModuleInterop=true,globalThisPolyfill=true,comments=false \
